...
// To add a new tool: add it to the relevant group's `tools` array.
// To add a new group: add a new entry to TOOL_GROUPS.
...
const TOOL_GROUPS: ToolGroup[] = [
  {
    ...
    icon: MousePointer2,
    tools: [
      { id: 'select', name: 'Выбор', icon: MousePointer2, mode: 'select' },
      { id: 'eraser', name: 'Ластик', icon: Eraser, mode: 'eraser' },
    ],
  },
  {
    id: 'geometry',
    ...
    icon: Dot,
    tools: [
      { id: 'geopoint', name: 'Точка', icon: Dot, mode: 'geopoint' },
  ...
  },
  {
    ...
    icon: Type,
    tools: [
      { id: 'text', name: 'Текст', icon: Type, mode: 'text' },
    ],
  },
  {
    id: 'interactive',
    ...
    accent: 'indigo',
    tools: [
      { id: 'interactive', name: 'Интерактив', icon: FunctionSquare, mode: 'interactive' },
    ],
  },
  {
    id: 'library',
    ...
    icon: Library,
    tools: [
      { id: 'library', name: 'Библиотека', icon: Library, mode: 'library' },
    ],
  },
  {
    id: 'projects',
    ...
    accent: 'emerald',
    tools: [
      { id: 'projects', name: 'Проекты', icon: FolderKanban, mode: 'projects' },
    ],
  },
  {
    id: 'challenge',
    ...
    accent: 'amber',
    tools: [
      { id: 'challenge', name: 'Задачи', icon: Brain, mode: 'challenge' },
    ],
  },
];
...
export const ToolSidebar: React.FC<ToolSidebarProps> = ({ onNew, onOpen, onSave, onExport }) => {
  ...
  return (
    <div
      ref={sidebarRef}
      className="w-16 bg-white border-r border-gray-200 flex flex-col items-center py-4 gap-2 relative"
    >
      {/* File operations */}
      <div className="flex flex-col gap-1 mb-4">
        <button onClick={onNew} className="p-2 rounded-lg hover:bg-gray-100 text-gray-600" title="Новый проект">
          <FilePlus size={20} />
        </button>
        <button onClick={onOpen} className="p-2 rounded-lg hover:bg-gray-100 text-gray-600" title="Открыть проект">
          <FolderOpen size={20} />
        </button>
        <button
          onClick={onSave}
        ...
        <button onClick={onExport} className="p-2 rounded-lg hover:bg-gray-100 text-gray-600" title="Экспорт">
          <Download size={20} />
        </button>
      </div>
      ...
      <div className="flex flex-col gap-1">
        {TOOL_GROUPS.map((group) => {
          ...
          // Show the active tool's icon when inside an expanded group
          const ActiveIcon = isGroupActive && group.tools.length > 1
          ...
          return (
            <div key={group.id} className="relative">
              ...
              {/* Popover — only for groups with multiple tools */}
